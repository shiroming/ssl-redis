...
void migrateCommand(redisClient *c) {
    ...
    if( server.ssl ) {
      ...
      if( fd < 0 ) {
           redisLog(REDIS_WARNING,"Unable to connect to MASTER via SSL: %s", server.neterr );
           return;
       }
     } else {
    ...
    /* Tranfer the query to the other node in 64K chunks. */
    {
        ...
        while ((towrite = sdslen(buf)-pos) > 0) {
            ...
            nwritten = syncWrite(fd,sslctn.ssl,buf+pos,towrite,timeout);
    ...
    /* Read back the reply. */
    {
        ...
        if (syncReadLine(fd, sslctn.ssl, buf1, sizeof(buf1), timeout) <= 0)
            goto socket_rd_err;
        if (syncReadLine(fd, sslctn.ssl, buf2, sizeof(buf2), timeout) <= 0)
            goto socket_rd_err;
        if (buf1[0] == '-' || buf2[0] == '-') {
